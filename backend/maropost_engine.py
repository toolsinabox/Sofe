...
class MaropostTemplateEngine:
    ...
    def select_page_template(self, page_type: PageType, model_data: Dict) -> Path:
        ...
        template_map = {
            ...
            PageType.CHECKOUT: 'cart/checkout.template.html',
            PageType.QUOTE: 'cart/quote.template.html',
    ...
    async def process_data_tags(self, content: str, context: Dict) -> str:
        ...
        replacements = {
            ...
            # Product tags (if in product context) - SHORT FORM
            'SKU': product.get('sku', ''),
            'sku': product.get('sku', ''),
            'name': product.get('name', page.get('title', '')),
            'description': product.get('description', page.get('description', '')),
            'short_description': (product.get('short_description') or (product.get('description', '') or '')[:150]) if product else '',
            ...
            'thumb': product.get('images', [''])[0] if product.get('images') and product.get('images')[0] else '',
            'thumbnail': product.get('thumbnail') or (product.get('images', [''])[0] if product.get('images') and product.get('images')[0] else ''),
            # Individual image slots (image1 through image12)
            'image1': product.get('images', [''])[0] if product.get('images') and len(product.get('images', [])) > 0 and product.get('images')[0] else '',
            'image2': product.get('images', [''])[1] if product.get('images') and len(product.get('images', [])) > 1 and product.get('images')[1] else '',
            'image3': product.get('images', [''])[2] if product.get('images') and len(product.get('images', [])) > 2 and product.get('images')[2] else '',
            'image4': product.get('images', [''])[3] if product.get('images') and len(product.get('images', [])) > 3 and product.get('images')[3] else '',
            'image5': product.get('images', [''])[4] if product.get('images') and len(product.get('images', [])) > 4 and product.get('images')[4] else '',
            'image6': product.get('images', [''])[5] if product.get('images') and len(product.get('images', [])) > 5 and product.get('images')[5] else '',
            'image7': product.get('images', [''])[6] if product.get('images') and len(product.get('images', [])) > 6 and product.get('images')[6] else '',
            'image8': product.get('images', [''])[7] if product.get('images') and len(product.get('images', [])) > 7 and product.get('images')[7] else '',
            'image9': product.get('images', [''])[8] if product.get('images') and len(product.get('images', [])) > 8 and product.get('images')[8] else '',
            'image10': product.get('images', [''])[9] if product.get('images') and len(product.get('images', [])) > 9 and product.get('images')[9] else '',
            'image11': product.get('images', [''])[10] if product.get('images') and len(product.get('images', [])) > 10 and product.get('images')[10] else '',
            'image12': product.get('images', [''])[11] if product.get('images') and len(product.get('images', [])) > 11 and product.get('images')[11] else '',
            ...
            'manufacturer': product.get('manufacturer', '') if product else '',
            'barcode': product.get('barcode', '') if product else '',
            'upc': product.get('barcode', '') if product else '',
            'mpn': product.get('mpn', '') if product else '',
            'rating': str(product.get('rating', 0)) if product else '0',
            'reviews_count': str(product.get('reviews_count', 0)) if product else '0',
            
            # Product tags - PREFIXED FORM (for backward compatibility with documentation)
            'product_id': product.get('id', ''),
            'product_name': product.get('name', ''),
            'product_description': product.get('description', ''),
            'product_short_description': (product.get('short_description') or (product.get('description', '') or '')[:150]) if product else '',
            'product_price': f"{product.get('price', 0):.2f}" if product else '',
            'product_price_formatted': f"{store.get('currency_symbol', '$')}{product.get('price', 0):.2f}" if product else '',
            'product_compare_price': f"{product.get('compare_price', 0):.2f}" if product.get('compare_price') else '',
            'product_rrp': f"{product.get('compare_price', 0):.2f}" if product.get('compare_price') else '',
            'product_sku': product.get('sku', ''),
            'product_barcode': product.get('barcode', ''),
            'product_upc': product.get('barcode', ''),
            'product_mpn': product.get('mpn', ''),
            'product_brand': product.get('brand', ''),
            'product_manufacturer': product.get('manufacturer', ''),
            'product_category': category.get('name', '') if category else '',
            'product_category_id': category.get('id', '') if category else '',
            'product_image': product.get('images', [''])[0] if product.get('images') and product.get('images')[0] else '',
            'product_thumbnail': product.get('thumbnail') or (product.get('images', [''])[0] if product.get('images') and product.get('images')[0] else ''),
            'product_images': ','.join(product.get('images', [])) if product else '',
            'product_stock': str(product.get('stock', 0)),
            'product_qty': str(product.get('stock', 0)),
            'product_in_stock': 'y' if product.get('stock', 0) > 0 else 'n',
            'product_weight': str(product.get('weight', '')) if product else '',
            'product_shipping_length': str(product.get('shipping_length', 0) or 0) if product else '0',
            'product_shipping_width': str(product.get('shipping_width', 0) or 0) if product else '0',
            'product_shipping_height': str(product.get('shipping_height', 0) or 0) if product else '0',
            'shipping_length': str(product.get('shipping_length', 0) or 0) if product else '0',
            'shipping_width': str(product.get('shipping_width', 0) or 0) if product else '0',
            ...
            'product_on_sale': 'y' if (product.get('compare_price') or 0) > (product.get('price') or 0) else 'n',
            'product_tax_class': product.get('tax_class', 'standard') if product else '',
            'product_shipping_class': product.get('shipping_class', '') if product else '',
            'product_meta_title': product.get('meta_title', '') if product else '',
            'product_meta_description': product.get('meta_description', '') if product else '',
            'product_tags': ','.join(product.get('tags', [])) if product else '',
            'product_created': product.get('created_at', '').isoformat() if product and product.get('created_at') else '',
            'product_updated': product.get('updated_at', '').isoformat() if product and product.get('updated_at') else '',
    ...
    async def render_page(
        ...
        try:
            ...
            html = await self.process_conditionals(html, context)  # Process conditionals FIRST (looks up [@tag@] from context)
            html = await self.process_data_tags(html, context)  # Then replace data tags
